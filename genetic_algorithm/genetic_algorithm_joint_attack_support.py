...
class JointAttackSupportGeneticAlgorithm:
    ...
    def __init__(
        ...
        self.train_dl = DataLoader(train_ds, batch_size=256, shuffle=True)
        self.x_val = x_val
        self.y_val = y_val
        self.X_te = x_test
        self.y_te = y_test
    ...
    def encode(self, parents: List[JASGBAG]) -> List[List[torch.Tensor]]:
    ...
    def decode(
        ...
        return [JASGBAG.from_flattened_mask_matrices(
    ...
    def create_population(self):
        ...
        for _ in range(self.params["population_size"]):
            self.population.append(
                JASGBAG.random_connectivity_init(
                    input_size=self.input_size,
                    hidden_size=self.params["hidden_neuron_size"],
                    output_size=self.output_size,
    ...
    def create_new_generation(self, elitist: List[JASGBAG], mutated_encodings: List[torch.Tensor]):
        ...
        for i in range(new_generation_size):
            child = JASGBAG.from_flattened_mask_matrices(
                input_size=self.input_size,
                hidden_size=self.params["hidden_neuron_size"],
                output_size=self.output_size,
    ...
    def fitness(
        ...
        for i, individual in enumerate(population):
            ...
            for epoch in range(self.params["number_epochs"]):
                ...
                individual.fitness = ( 1 - self.params["lambda"]) * accuracy + self.params["lambda"] * (1 / individual.total_num_conn())
                individual.accuracy = accuracy
                individual.sparsity = (1 / individual.total_num_conn())
...
#
        ...
        with open(host_name + start_time + "joint" + file_name, "a") as file:
