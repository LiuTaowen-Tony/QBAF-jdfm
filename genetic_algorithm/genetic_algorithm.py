...
from genetic_algorithm.GBAG import GBAG as GBAG
...
now = datetime.now()
start_run_time = now.strftime("%d_%H_%M_%S")
...
def accuracy_f(*args, **kw):
    return accuracy(*args, **kw)
...
def get_num_conn(individual: GBAG):
    return  (
        individual.sparse_linear1.connectivity.shape[1]
        + individual.sparse_linear2.connectivity.shape[1]
    )
...
class GeneticAlgorithm:
            ...
            for epoch in range(self.params["number_epochs"]):
                if minibatch:
                    ...
                    for nb, (x_batch, y_batch) in enumerate(train_dl):
                        ...
                        batch_accuracy += accuracy_f(y_pred_train, y_batch).item()
                    ...
                    accuracy = accuracy_f(y_pred_train, y_train).item()
                individual.fitness = (
                ...
                with torch.no_grad():
                    ...
                    accuracy_val = accuracy_f(y_pred_val, y_val).item()
                score = -val_loss.item()
    ...
    def run(
        ...
        self.fitness(self.population, X_tr, y_tr, X_val, y_val)
        ...
        for g in range(self.params["number_generations"]):
            ...
            n_conns = [get_num_conn(indiv) for indiv in self.population]
        ...
        with torch.no_grad():
            ...
            test_accuracy = accuracy_f(y_pred, y_te)
        ...
        this_file = os.uname().nodename + start_run_time + file_name
